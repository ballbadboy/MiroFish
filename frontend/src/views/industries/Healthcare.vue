  ...
  <div class="page-wrap">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        <span class="nav-subtitle">Healthcare &amp; Medical Intelligence</span>
      </div>

      <div class="nav-right">
        <a class="nav-back" @click.prevent="router.push('/')">← Back to Platform</a>
        <button class="btn-sim" @click="router.push('/new')">Start Healthcare Simulation →</button>
      </div>
    </nav>
    ...
    <section class="hero-section">
      <div class="hero-left">
        <div class="breadcrumb">Platform / Industries / Healthcare</div>

        <div class="industry-badge">⚕ HEALTHCARE &amp; MEDICAL</div>

        <h1 class="hero-h1">Predict Patient Outcomes Before They Happen</h1>

        <p class="hero-sub">
          From epidemic modeling to ER triage optimization — ENDORA simulates
          million-scale patient populations so healthcare systems can act before
          crisis strikes.
        </p>
        ...
        <div class="hero-stats-inline">
          <div class="inline-stat">
            ...
            <span class="inline-lbl">Patient Agents</span>
          </div>
        ...
        <button class="btn-cta-teal" @click="router.push('/new')">
      ...
      <div class="hero-right">
        <div class="terminal-card">
          <div class="terminal-header">
            ...
            <span class="terminal-title">◈ LIVE SIMULATION / ICU_OVERFLOW_MODEL_03</span>
          </div>
    ...
    <section class="section sect-mid">
      <div class="sect-inner">
        ...
        <h2 class="sect-h2">Where ENDORA Transforms Healthcare</h2>
        ...
        <div class="compliance-box">
          <div class="compliance-text">
            ...
            <p class="compliance-body">
              ENDORA Healthcare operates on de-identified data only. All simulations run in
              isolated environments. HIPAA-aligned architecture. No patient PII is ever stored
              or transmitted.
            </p>
          </div>
    ...
    <section class="section sect-dark cta-section">
      <div class="sect-inner cta-inner">
        <h2 class="cta-h2">Ready to simulate your healthcare system?</h2>
        <p class="cta-sub">Join hospitals, insurers, and public health agencies already using ENDORA.</p>
        <button class="btn-cta-teal btn-cta-large" @click="router.push('/new')">
          Start Healthcare Simulation →
        </button>
        <div class="cta-contact">
          Contact our Healthcare team:
          <a class="cta-email" href="mailto:healthcare@endora.ai">healthcare@endora.ai</a>
        </div>
      </div>
    </section>
...
const logLines = [
  { ts: '[00:00:01]', msg: 'Initializing 847,293 patient agents...', cls: 'log-normal' },
  { ts: '[00:00:03]', msg: 'Seeding ICU capacity: 420 beds', cls: 'log-normal' },
  { ts: '[00:00:07]', msg: 'Injecting seasonal flu variant...', cls: 'log-warn' },
  { ts: '[00:00:12]', msg: 'Agent interactions: 2,847,103', cls: 'log-normal' },
  { ts: '[00:00:18]', msg: 'Overflow predicted at T+72h (confidence: 87%)', cls: 'log-alert' },
  { ts: '[00:00:21]', msg: 'Recommended: activate surge protocol B', cls: 'log-success' },
]
...
const useCases = [
  {
    icon: '◉',
    title: 'Epidemic & Outbreak Modeling',
    desc: 'Simulate disease propagation across population networks. Predict peak hospitalization dates, optimal intervention windows, and vaccine rollout strategies.',
    tag: 'PUBLIC HEALTH · EPIDEMIOLOGY',
  },
  {
    icon: '▣',
    title: 'Hospital Operations',
    desc: 'Model patient flow from admission to discharge. Predict ER bottlenecks, optimize bed allocation, and simulate staff scheduling impacts before implementation.',
    tag: 'HOSPITAL MGMT · OPERATIONS',
  },
  {
    icon: '⬡',
    title: 'Clinical Trial Simulation',
    desc: 'Run virtual clinical populations to predict trial outcomes, adverse event rates, and optimal cohort sizes before committing to expensive in-person trials.',
    tag: 'PHARMA · R&D',
  },
  {
    icon: '◈',
    title: 'Drug Demand Forecasting',
    desc: 'Simulate regional medication demand under various outbreak scenarios. Prevent stockouts. Optimize supply chain for critical drugs.',
    tag: 'SUPPLY CHAIN · PHARMA',
  },
  {
    icon: '◫',
    title: 'Mental Health Crisis Prediction',
    desc: 'Model community mental health trajectories under economic stress, natural disaster, or policy changes. Enable proactive resource allocation.',
    tag: 'MENTAL HEALTH · POLICY',
  },
  {
    icon: '◉',
    title: 'Surgical Scheduling Optimization',
    desc: 'Simulate surgical queue dynamics to minimize cancellations, reduce patient wait times, and maximize OR utilization across hospital networks.',
    tag: 'OPERATIONS · SURGICAL',
  },
]
...
const workflowSteps = [
  {
    label: 'UPLOAD',
    desc: 'Feed ENDORA your patient records (de-identified), operational data, or medical literature',
  },
  {
    label: 'ONTOLOGIZE',
    desc: 'ENDORA extracts disease pathways, patient personas, clinical protocols, and hospital network topology',
  },
  {
    label: 'SIMULATE',
    desc: 'Run million-scale agent simulations of patient populations interacting with your healthcare system',
  },
  {
    label: 'PREDICT',
    desc: 'Receive probabilistic outcome reports with intervention recommendations and confidence intervals',
  },
]

// ── Metrics ───────────────────────────────────────────────
const metrics = [
  { val: '847,293', label: 'avg agents per healthcare sim' },
  { val: '87%',     label: 'avg prediction confidence' },
  { val: '< $3',    label: 'avg cost per simulation' },
  { val: '72h',     label: 'earliest outbreak prediction lead time' },
]
...
.page-wrap {
  ...
  --teal: #2dd4bf; --teal-bg: rgba(45,212,191,0.08); --teal-border: rgba(45,212,191,0.25);
...
.btn-sim:hover { opacity: 0.88; transform: translateY(-1px); box-shadow: 0 0 32px rgba(45,212,191,0.35); }
...
.industry-badge {
  ...
  border-radius: 4px; padding: 6px 14px; background: var(--teal-bg);
}
.hero-h1 {
...
.hero-stats-inline {
  ...
  background: rgba(45,212,191,0.04); align-self: flex-start; overflow: hidden;
}
.inline-stat { display: flex; flex-direction: column; padding: 12px 24px; gap: 2px; }
.inline-val { font-family: var(--mono); font-size: 1rem; font-weight: 700; color: var(--teal); }
.inline-lbl { font-size: 0.7rem; color: var(--muted); letter-spacing: 0.06em; text-transform: uppercase; }
.inline-sep { width: 1px; height: 36px; background: var(--teal-border); flex-shrink: 0; }
...
.btn-cta-teal:hover { opacity: 0.9; transform: translateY(-2px); box-shadow: 0 0 48px rgba(45,212,191,0.45); }

/* Terminal card */
.hero-right { display: flex; align-items: center; justify-content: center; }
.terminal-card {
  width: 100%; max-width: 520px;
  background: #0a0e15; border: 1px solid var(--teal-border);
  border-radius: 14px; overflow: hidden;
  box-shadow: 0 0 0 1px rgba(45,212,191,0.08), 0 0 48px rgba(45,212,191,0.12), 0 24px 64px rgba(0,0,0,0.6);
}
.terminal-header {
  display: flex; align-items: center; gap: 8px;
  padding: 12px 16px; background: rgba(45,212,191,0.04);
...
.log-ts {
  ...
  color: rgba(45,212,191,0.45); white-space: nowrap; flex-shrink: 0;
}
...
.sect-label { font-family: var(--mono); font-size: 0.72rem; font-variant: small-caps; letter-spacing: 0.18em; color: var(--teal); margin-bottom: 16px; }
...
.case-card:hover {
  ...
  box-shadow: 0 0 0 1px rgba(45,212,191,0.08), 0 16px 48px rgba(0,0,0,0.4);
  transform: translateY(-2px);
}
.case-icon { font-size: 1.5rem; line-height: 1; color: var(--teal); }
...
.wf-connector {
  ...
  height: 1px; border-top: 1px dashed rgba(45,212,191,0.3);
...
.metric-val { font-family: var(--mono); font-size: 1.6rem; font-weight: 700; color: var(--teal); }
...
.compliance-box {
  border: 1px solid var(--teal-border); border-left: 3px solid var(--teal);
  border-radius: 12px; background: var(--teal-bg);
...
.btn-cta-large:hover { box-shadow: 0 0 64px rgba(45,212,191,0.5); }
.cta-contact { font-family: var(--mono); font-size: 0.75rem; color: var(--muted); letter-spacing: 0.05em; }
.cta-email {
  color: var(--teal); text-decoration: none; margin-left: 6px;
