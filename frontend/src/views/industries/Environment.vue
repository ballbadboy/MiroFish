  ...
  <div class="page-wrap">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        <span class="nav-subtitle">Environment &amp; Climate Risk</span>
      </div>

      <div class="nav-right">
        <a class="nav-back" @click.prevent="router.push('/')">← Back to Platform</a>
        <button class="btn-sim" @click="router.push('/new')">Start Climate Simulation →</button>
      </div>
    </nav>
    ...
    <section class="hero-section">
      <div class="hero-left">
        <div class="breadcrumb">Platform / Industries / Environment</div>

        <div class="industry-badge">◉ ENVIRONMENT &amp; CLIMATE</div>

        <h1 class="hero-h1">Model Climate Impact Before It Strikes</h1>

        <p class="hero-sub">
          From flood modeling to ESG policy simulation — ENDORA creates living
          models of environmental systems and human response to predict outcomes
          and optimize interventions.
        </p>

        <div class="hero-stats-inline">
          <div class="inline-stat">
            <span class="inline-val">8M+</span>
            <span class="inline-lbl">Community Agents</span>
          </div>
          <div class="inline-sep"></div>
          <div class="inline-stat">
            <span class="inline-val">72h</span>
            <span class="inline-lbl">Lead Time</span>
          </div>
        </div>

        <button class="btn-cta-emerald" @click="router.push('/new')">
      ...
      <div class="hero-right">
        <div class="terminal-card">
          <div class="terminal-header">
            ...
            <span class="terminal-title">◉ CLIMATE MODEL / FLOOD_CHAOPRAYA_2025</span>
          </div>
    ...
    <section class="section sect-mid">
      <div class="sect-inner">
        ...
        <h2 class="sect-h2">Where ENDORA Transforms Environmental Intelligence</h2>
      ...
      <div class="sect-inner">
        <div class="compliance-box">
          <div class="compliance-text">
            <div class="sect-label" style="margin-bottom: 12px;">◉ DATA &amp; TRANSPARENCY</div>
            <p class="compliance-body">
              ENDORA Environment uses publicly available satellite imagery, open weather data,
              and aggregated demographic models. All simulations support evidence-based
              policymaking without individual surveillance.
            </p>
          </div>
    ...
    <section class="section sect-dark cta-section">
      <div class="sect-inner cta-inner">
        <h2 class="cta-h2">Ready to model climate impact?</h2>
        <p class="cta-sub">Join environmental agencies, insurers, and ESG teams already using ENDORA.</p>
        <button class="btn-cta-emerald btn-cta-large" @click="router.push('/new')">
          Start Climate Simulation →
        </button>
        <div class="cta-contact">
          Contact our Environment team:
          <a class="cta-email" href="mailto:climate@endora.ai">climate@endora.ai</a>
        </div>
      </div>
    </section>
...
const logLines = [
  { ts: '[00:00:01]', msg: 'Initializing 4,500,000 community agents...', cls: 'log-normal' },
  { ts: '[00:00:05]', msg: 'Loading satellite imagery: Chao Phraya basin...', cls: 'log-normal' },
  { ts: '[00:00:10]', msg: 'Injecting monsoon scenario: 150% avg rainfall', cls: 'log-warn' },
  { ts: '[00:00:15]', msg: 'Flood zone expansion: +47% at T+96h', cls: 'log-alert' },
  { ts: '[00:00:20]', msg: 'Evacuation compliance: 62% predicted', cls: 'log-normal' },
  { ts: '[00:00:24]', msg: 'Recommended: pre-deploy shelters in Zone C-7', cls: 'log-success' },
]
...
const useCases = [
  {
    icon: '◉',
    title: 'Flood & Disaster Modeling',
    desc: 'Simulate flood propagation, wildfire spread, or earthquake impact across populated areas. Predict evacuation patterns and resource needs.',
    tag: 'DISASTER · EMERGENCY',
  },
  {
    icon: '◈',
    title: 'ESG Policy Impact',
    desc: 'Model how environmental regulations affect corporate behavior, community livelihood, and ecosystem health simultaneously.',
    tag: 'ESG · COMPLIANCE',
  },
  {
    icon: '▣',
    title: 'Carbon Credit Forecasting',
    desc: 'Simulate global carbon credit markets under different regulatory frameworks. Predict pricing dynamics and trading opportunities.',
    tag: 'CARBON · TRADING',
  },
  {
    icon: '◫',
    title: 'Community Climate Response',
    desc: 'Model how communities adapt to rising temperatures, water scarcity, or air quality decline. Predict migration and infrastructure demand.',
    tag: 'ADAPTATION · MIGRATION',
  },
  {
    icon: '⬡',
    title: 'Renewable Energy Transition',
    desc: 'Simulate the transition from fossil to renewable energy at regional scale. Predict grid stability, job displacement, and economic impact.',
    tag: 'ENERGY · TRANSITION',
  },
  {
    icon: '⚕',
    title: 'Biodiversity Impact Assessment',
    desc: 'Model ecosystem response to development, pollution, or conservation policy. Predict species migration and habitat viability.',
    tag: 'ECOLOGY · CONSERVATION',
  },
]
...
const workflowSteps = [
  {
    label: 'INGEST',
    desc: 'Feed ENDORA satellite imagery, weather data, sensor readings, or policy documents',
  },
  {
    label: 'MODEL',
    desc: 'ENDORA builds environmental ontology: terrain, watersheds, population clusters, and infrastructure',
  },
  {
    label: 'SIMULATE',
    desc: 'Run million-scale agent simulation of community response to climate scenarios',
  },
  {
    label: 'PREDICT',
    desc: 'Receive impact forecasts, intervention recommendations, and risk maps with confidence levels',
  },
]

// ── Metrics ───────────────────────────────────────────────
const metrics = [
  { val: '4,500,000', label: 'avg agents per climate sim' },
  { val: '62%',       label: 'evacuation compliance prediction' },
  { val: '< $5',      label: 'avg cost per simulation' },
  { val: '96h',       label: 'earliest disaster prediction window' },
]
...
.page-wrap {
  ...
  --emerald: #10b981; --emerald-bg: rgba(16,185,129,0.08); --emerald-border: rgba(16,185,129,0.25);
...
.btn-sim {
  background: var(--emerald); color: #080c12; border: none;
...
.btn-sim:hover { opacity: 0.88; transform: translateY(-1px); box-shadow: 0 0 32px rgba(16,185,129,0.35); }
...
.industry-badge {
  ...
  color: var(--emerald); border: 1px solid var(--emerald-border);
  border-radius: 4px; padding: 6px 14px; background: var(--emerald-bg);
}
.hero-h1 {
...
.hero-stats-inline {
  ...
  background: rgba(16,185,129,0.04); align-self: flex-start; overflow: hidden;
}
.inline-stat { display: flex; flex-direction: column; padding: 12px 24px; gap: 2px; }
.inline-val { font-family: var(--mono); font-size: 1rem; font-weight: 700; color: var(--emerald); }
.inline-lbl { font-size: 0.7rem; color: var(--muted); letter-spacing: 0.06em; text-transform: uppercase; }
.inline-sep { width: 1px; height: 36px; background: var(--emerald-border); flex-shrink: 0; }
...
.btn-cta-emerald:hover { opacity: 0.9; transform: translateY(-2px); box-shadow: 0 0 48px rgba(16,185,129,0.45); }

/* Terminal card */
.hero-right { display: flex; align-items: center; justify-content: center; }
.terminal-card {
  width: 100%; max-width: 520px;
  background: #0a0e15; border: 1px solid var(--emerald-border);
  border-radius: 14px; overflow: hidden;
  box-shadow: 0 0 0 1px rgba(16,185,129,0.08), 0 0 48px rgba(16,185,129,0.12), 0 24px 64px rgba(0,0,0,0.6);
}
.terminal-header {
...
.log-ts {
  ...
  color: rgba(16,185,129,0.45); white-space: nowrap; flex-shrink: 0;
}
...
.cursor-blink {
  font-family: var(--mono); font-size: 0.85rem; color: var(--emerald);
...
.sect-label { font-family: var(--mono); font-size: 0.72rem; font-variant: small-caps; letter-spacing: 0.18em; color: var(--emerald); margin-bottom: 16px; }
...
.case-card:hover {
  ...
  box-shadow: 0 0 0 1px rgba(16,185,129,0.08), 0 16px 48px rgba(0,0,0,0.4);
  transform: translateY(-2px);
}
.case-icon { font-size: 1.5rem; line-height: 1; color: var(--emerald); }
...
.case-tag {
  font-family: var(--mono); font-size: 0.62rem; color: var(--emerald);
...
.wf-connector {
  ...
  height: 1px; border-top: 1px dashed rgba(16,185,129,0.3);
...
.metric-val { font-family: var(--mono); font-size: 1.6rem; font-weight: 700; color: var(--emerald); }
...
.compliance-box {
  border: 1px solid var(--emerald-border); border-left: 3px solid var(--emerald);
  border-radius: 12px; background: var(--emerald-bg);
...
.badge-item {
  ...
  color: var(--emerald); letter-spacing: 0.06em;
...
.btn-cta-large:hover { box-shadow: 0 0 64px rgba(16,185,129,0.5); }
.cta-contact { font-family: var(--mono); font-size: 0.75rem; color: var(--muted); letter-spacing: 0.05em; }
.cta-email {
  color: var(--emerald); text-decoration: none; margin-left: 6px;
