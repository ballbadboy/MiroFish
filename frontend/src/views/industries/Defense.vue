  ...
  <div class="page-wrap">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        <span class="nav-subtitle">Defense &amp; Security Intelligence</span>
      </div>

      <div class="nav-right">
        <a class="nav-back" @click.prevent="router.push('/')">← Back to Platform</a>
        <button class="btn-sim" @click="router.push('/new')">Run Threat Simulation →</button>
      </div>
    </nav>
    ...
    <section class="hero-section">
      <div class="hero-left">
        <div class="breadcrumb">Platform / Industries / Defense</div>

        <div class="industry-badge">⬡ DEFENSE &amp; SECURITY</div>

        <h1 class="hero-h1">Wargame the Future Before You Deploy</h1>

        <p class="hero-sub">
          From narrative warfare simulation to threat propagation modeling —
          ENDORA builds adversarial agent populations to stress-test strategies
          before they meet reality.
        </p>

        <div class="hero-stats-inline">
          <div class="inline-stat">
            <span class="inline-val">10M+</span>
            <span class="inline-lbl">Threat Agents</span>
          </div>
          <div class="inline-sep"></div>
          <div class="inline-stat">
            <span class="inline-val">Classified</span>
            <span class="inline-lbl">Ready Architecture</span>
          </div>
        </div>

        <button class="btn-cta-red" @click="router.push('/new')">
      ...
      <div class="hero-right">
        <div class="terminal-card">
          <div class="terminal-header">
            ...
            <span class="terminal-title">⬡ THREAT SIMULATION / NARWAR_SEA_SCENARIO_07</span>
          </div>
    ...
    <section class="section sect-mid">
      <div class="sect-inner">
        ...
        <h2 class="sect-h2">Where ENDORA Transforms Defense</h2>
      ...
      <div class="sect-inner">
        <div class="compliance-box">
          <div class="compliance-text">
            <div class="sect-label" style="margin-bottom: 12px;">⬡ SECURITY &amp; DEPLOYMENT</div>
            <p class="compliance-body">
              ENDORA Defense operates in air-gapped deployment mode for classified
              environments. On-premise installation available. All simulations run
              in isolated compute with no external data transmission.
            </p>
          </div>
          <div class="compliance-badges">
            <div class="badge-item" v-for="b in securityBadges" :key="b">
    ...
    <section class="section sect-dark cta-section">
      <div class="sect-inner cta-inner">
        <h2 class="cta-h2">Ready to wargame the future?</h2>
        <p class="cta-sub">Contact us for a classified capability briefing.</p>
        <button class="btn-cta-red btn-cta-large" @click="scrollToCta">
          Request Capability Briefing →
        </button>
        <div class="cta-contact">
          Contact our Defense team:
          <a class="cta-email" href="mailto:defense@endora.ai">defense@endora.ai</a>
        </div>
      </div>
    </section>
...
const logLines = [
  { ts: '[00:00:01]', msg: 'Deploying 2,100,000 adversarial agents...', cls: 'log-normal' },
  { ts: '[00:00:05]', msg: 'Loading OSINT geospatial feed (SCS region)...', cls: 'log-normal' },
  { ts: '[00:00:10]', msg: 'Injecting narrative warfare: 847 bot accounts', cls: 'log-warn' },
  { ts: '[00:00:15]', msg: 'Escalation probability: 34% → 67% at T+48h', cls: 'log-alert' },
  { ts: '[00:00:20]', msg: 'Counter-narrative effectiveness: 72% suppression', cls: 'log-normal' },
  { ts: '[00:00:24]', msg: 'Recommended: deploy Protocol ECHO-7 at T+12h', cls: 'log-success' },
]
...
const useCases = [
  {
    icon: '◫',
    title: 'Narrative Warfare Simulation',
    desc: 'Model information warfare campaigns across social platforms. Simulate bot swarms, counter-narratives, and public opinion manipulation at scale.',
    tag: 'INFO OPS · PSYOPS',
  },
  {
    icon: '◉',
    title: 'Threat Propagation Modeling',
    desc: 'Simulate how threats propagate through networks — from cyber intrusion chains to kinetic conflict escalation scenarios.',
    tag: 'CYBER · KINETIC',
  },
  {
    icon: '⬡',
    title: 'Strategic Wargaming',
    desc: 'Run red-team/blue-team simulations with AI-driven adversaries. Test doctrine, resource allocation, and multi-domain strategy.',
    tag: 'WARGAME · STRATEGY',
  },
  {
    icon: '◈',
    title: 'OSINT Fusion & Analysis',
    desc: 'Combine open-source intelligence streams into agent-based models. Predict adversary intent from behavioral patterns.',
    tag: 'OSINT · HUMINT',
  },
  {
    icon: '▣',
    title: 'Supply Line Disruption',
    desc: 'Simulate logistics network resilience under denial scenarios. Identify critical chokepoints before adversaries exploit them.',
    tag: 'LOGISTICS · C4ISR',
  },
  {
    icon: '⚕',
    title: 'Alliance & Coalition Modeling',
    desc: 'Model multi-party coalition dynamics. Simulate defection risk, burden-sharing conflicts, and joint operation scenarios.',
    tag: 'COALITION · DIPLOMACY',
  },
]
...
const workflowSteps = [
  {
    label: 'INGEST',
    desc: 'Feed ENDORA OSINT data, geospatial feeds, SIGINT intercepts, or scenario briefs',
  },
  {
    label: 'THREAT MODEL',
    desc: 'ENDORA generates adversary personas, capability profiles, and intent models',
  },
  {
    label: 'WARGAME',
    desc: 'Run multi-domain agent simulation with red/blue/gray force interactions',
  },
  {
    label: 'ASSESS',
    desc: 'Receive probabilistic threat assessments with course-of-action recommendations',
  },
]

// ── Metrics ───────────────────────────────────────────────
const metrics = [
  { val: '2,100,000', label: 'avg agents per threat sim' },
  { val: '67%',       label: 'escalation detection rate' },
  { val: '72%',       label: 'counter-narrative effectiveness' },
  { val: '48h',       label: 'earliest threat prediction window' },
]
...
.btn-sim:hover { opacity: 0.88; transform: translateY(-1px); box-shadow: 0 0 32px rgba(239,68,68,0.35); }
...
.industry-badge {
  ...
  border-radius: 4px; padding: 6px 14px; background: var(--red-bg);
}
.hero-h1 {
...
.hero-stats-inline {
  ...
  background: rgba(239,68,68,0.04); align-self: flex-start; overflow: hidden;
}
.inline-stat { display: flex; flex-direction: column; padding: 12px 24px; gap: 2px; }
.inline-val { font-family: var(--mono); font-size: 1rem; font-weight: 700; color: var(--red); }
.inline-lbl { font-size: 0.7rem; color: var(--muted); letter-spacing: 0.06em; text-transform: uppercase; }
.inline-sep { width: 1px; height: 36px; background: var(--red-border); flex-shrink: 0; }
...
.btn-cta-red:hover { opacity: 0.9; transform: translateY(-2px); box-shadow: 0 0 48px rgba(239,68,68,0.45); }

/* Terminal card */
.hero-right { display: flex; align-items: center; justify-content: center; }
.terminal-card {
  width: 100%; max-width: 520px;
  background: #0a0e15; border: 1px solid var(--red-border);
  border-radius: 14px; overflow: hidden;
  box-shadow: 0 0 0 1px rgba(239,68,68,0.08), 0 0 48px rgba(239,68,68,0.12), 0 24px 64px rgba(0,0,0,0.6);
}
.terminal-header {
...
.log-ts {
  ...
  color: rgba(239,68,68,0.45); white-space: nowrap; flex-shrink: 0;
}
...
.sect-label { font-family: var(--mono); font-size: 0.72rem; font-variant: small-caps; letter-spacing: 0.18em; color: var(--red); margin-bottom: 16px; }
...
.case-card:hover {
  ...
  box-shadow: 0 0 0 1px rgba(239,68,68,0.08), 0 16px 48px rgba(0,0,0,0.4);
  transform: translateY(-2px);
}
.case-icon { font-size: 1.5rem; line-height: 1; color: var(--red); }
...
.wf-connector {
  ...
  height: 1px; border-top: 1px dashed rgba(239,68,68,0.3);
...
.metric-val { font-family: var(--mono); font-size: 1.6rem; font-weight: 700; color: var(--red); }
...
.compliance-box {
  ...
  border-radius: 12px; background: var(--red-bg);
...
.btn-cta-large:hover { box-shadow: 0 0 64px rgba(239,68,68,0.5); }
.cta-contact { font-family: var(--mono); font-size: 0.75rem; color: var(--muted); letter-spacing: 0.05em; }
.cta-email {
  color: var(--red); text-decoration: none; margin-left: 6px;
