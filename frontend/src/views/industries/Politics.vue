  ...
  <div class="page-wrap">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        <span class="nav-subtitle">Political &amp; Policy Intelligence</span>
      </div>

      <div class="nav-right">
        <a class="nav-back" @click.prevent="router.push('/')">← Back to Platform</a>
        <button class="btn-sim" @click="router.push('/new')">Run Policy Simulation →</button>
      </div>
    </nav>
    ...
    <section class="hero-section">
      <div class="hero-left">
        <div class="breadcrumb">Platform / Industries / Politics</div>

        <div class="industry-badge">◫ POLITICAL &amp; POLICY</div>

        <h1 class="hero-h1">Simulate Policy Before You Legislate</h1>

        <p class="hero-sub">
          From election modeling to public opinion simulation — ENDORA builds
          living models of voter populations and policy stakeholders to predict
          political outcomes with precision.
        </p>

        <div class="hero-stats-inline">
          <div class="inline-stat">
            <span class="inline-val">15M+</span>
            <span class="inline-lbl">Voter Agents</span>
          </div>
          <div class="inline-sep"></div>
          <div class="inline-stat">
            <span class="inline-val">Multi-Party</span>
            <span class="inline-lbl">Modeling</span>
          </div>
        </div>

        <button class="btn-cta-purple" @click="router.push('/new')">
      ...
      <div class="hero-right">
        <div class="terminal-card">
          <div class="terminal-header">
            ...
            <span class="terminal-title">◫ POLICY MODEL / MINIMUM_WAGE_IMPACT_2025</span>
          </div>
    ...
    <section class="section sect-mid">
      <div class="sect-inner">
        ...
        <h2 class="sect-h2">Where ENDORA Transforms Politics</h2>
        ...
        <div class="compliance-box">
          <div class="compliance-text">
            ...
            <p class="compliance-body">
              ENDORA Political operates on publicly available polling data, census records,
              and open parliamentary data. All simulations produce probabilistic forecasts
              for research and advisory purposes only.
            </p>
          </div>
    ...
    <section class="section sect-dark cta-section">
      <div class="sect-inner cta-inner">
        <h2 class="cta-h2">Ready to simulate political outcomes?</h2>
        <p class="cta-sub">Join policy advisors, think tanks, and government agencies already using ENDORA.</p>
        <button class="btn-cta-purple btn-cta-large" @click="router.push('/new')">
          Run Policy Simulation →
        </button>
        <div class="cta-contact">
          Contact our Policy team:
          <a class="cta-email" href="mailto:policy@endora.ai">policy@endora.ai</a>
        </div>
      </div>
    </section>
...
const logLines = [
  { ts: '[00:00:01]', msg: 'Initializing 12,400,000 citizen agents...', cls: 'log-normal' },
  { ts: '[00:00:05]', msg: 'Loading demographic data: national census 2023', cls: 'log-normal' },
  { ts: '[00:00:10]', msg: 'Injecting policy: minimum wage +15% scenario', cls: 'log-warn' },
  { ts: '[00:00:15]', msg: 'SME sentiment shift: -8.2% approval', cls: 'log-alert' },
  { ts: '[00:00:20]', msg: 'Labor market response: +3.1% formal employment', cls: 'log-normal' },
  { ts: '[00:00:24]', msg: 'Net approval forecast: 54% ± 4% (conf: 76%)', cls: 'log-success' },
]
...
const useCases = [
  {
    ...
    desc: 'Simulate voter behavior across demographic segments. Predict turnout, swing districts, and the impact of campaign strategies on final outcomes.',
    tag: 'ELECTIONS · CAMPAIGNS',
  },
  {
    icon: '◈',
    title: 'Policy Impact Assessment',
    desc: 'Model how proposed legislation affects different population segments. Predict public approval, economic impact, and unintended consequences.',
    tag: 'LEGISLATION · IMPACT',
  },
  {
    icon: '◉',
    title: 'Public Opinion Tracking',
    desc: 'Simulate how news events, scandals, and policy announcements ripple through voter populations. Predict opinion shifts before polls catch up.',
    tag: 'OPINION · SENTIMENT',
  },
  {
    icon: '⬡',
    title: 'Coalition Dynamics',
    desc: 'Model multi-party coalition formation, stability, and breakdown scenarios. Predict which alliances hold under pressure.',
    tag: 'COALITION · PARLIAMENT',
  },
  {
    icon: '▣',
    title: 'Crisis Communication',
    desc: 'Simulate public response to government crisis messaging. Test communication strategies before deployment during emergencies.',
    tag: 'CRISIS · COMMS',
  },
  {
    icon: '⚕',
    title: 'Regulatory Impact Modeling',
    desc: 'Model how new regulations affect industry behavior, employment, and economic growth across regions and sectors.',
    tag: 'REGULATION · ECONOMIC',
  },
]
...
const workflowSteps = [
  {
    label: 'INGEST',
    desc: 'Feed ENDORA polling data, parliamentary records, social media feeds, or policy documents',
  },
  {
    label: 'MODEL',
    desc: 'ENDORA creates citizen personas: voter segments, interest groups, media influencers, and policy stakeholders',
  },
  {
    label: 'SIMULATE',
    desc: 'Run million-scale agent simulation of public response to your policy scenario',
  },
  {
    label: 'PREDICT',
    desc: 'Receive approval forecasts, demographic breakdowns, and risk assessments with confidence intervals',
  },
]

// ── Metrics ───────────────────────────────────────────────
const metrics = [
  { val: '12,400,000', label: 'avg agents per policy sim' },
  { val: '76%',        label: 'avg prediction confidence' },
  { val: '< $5',       label: 'avg cost per simulation' },
  { val: '54%',        label: 'avg approval prediction accuracy' },
]
...
.page-wrap {
  ...
  --accent: #8b5cf6; --accent-lt: #c4b5fd;
  --purple: #8b5cf6; --purple-bg: rgba(139,92,246,0.08); --purple-border: rgba(139,92,246,0.25);
...
.btn-sim:hover { opacity: 0.88; transform: translateY(-1px); box-shadow: 0 0 32px rgba(139,92,246,0.35); }
...
.industry-badge {
  ...
  border-radius: 4px; padding: 6px 14px; background: var(--purple-bg);
}
.hero-h1 {
...
.hero-stats-inline {
  ...
  background: rgba(139,92,246,0.04); align-self: flex-start; overflow: hidden;
}
.inline-stat { display: flex; flex-direction: column; padding: 12px 24px; gap: 2px; }
.inline-val { font-family: var(--mono); font-size: 1rem; font-weight: 700; color: var(--purple); }
.inline-lbl { font-size: 0.7rem; color: var(--muted); letter-spacing: 0.06em; text-transform: uppercase; }
.inline-sep { width: 1px; height: 36px; background: var(--purple-border); flex-shrink: 0; }
...
.btn-cta-purple:hover { opacity: 0.9; transform: translateY(-2px); box-shadow: 0 0 48px rgba(139,92,246,0.45); }

/* Terminal card */
.hero-right { display: flex; align-items: center; justify-content: center; }
.terminal-card {
  width: 100%; max-width: 520px;
  background: #0a0e15; border: 1px solid var(--purple-border);
  border-radius: 14px; overflow: hidden;
  box-shadow: 0 0 0 1px rgba(139,92,246,0.08), 0 0 48px rgba(139,92,246,0.12), 0 24px 64px rgba(0,0,0,0.6);
}
.terminal-header {
...
.log-ts {
  ...
  color: rgba(139,92,246,0.45); white-space: nowrap; flex-shrink: 0;
}
...
.sect-label { font-family: var(--mono); font-size: 0.72rem; font-variant: small-caps; letter-spacing: 0.18em; color: var(--purple); margin-bottom: 16px; }
...
.case-card:hover {
  ...
  box-shadow: 0 0 0 1px rgba(139,92,246,0.08), 0 16px 48px rgba(0,0,0,0.4);
  transform: translateY(-2px);
}
.case-icon { font-size: 1.5rem; line-height: 1; color: var(--purple); }
...
.case-tag {
  font-family: var(--mono); font-size: 0.62rem; color: var(--purple);
...
.wf-connector {
  ...
  height: 1px; border-top: 1px dashed rgba(139,92,246,0.3);
...
.metric-val { font-family: var(--mono); font-size: 1.6rem; font-weight: 700; color: var(--purple); }
...
.compliance-box {
  ...
  border-radius: 12px; background: var(--purple-bg);
...
.btn-cta-large:hover { box-shadow: 0 0 64px rgba(139,92,246,0.5); }
.cta-contact { font-family: var(--mono); font-size: 0.75rem; color: var(--muted); letter-spacing: 0.05em; }
.cta-email {
  color: var(--purple); text-decoration: none; margin-left: 6px;
